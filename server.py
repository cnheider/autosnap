...
from model.service import Caffe2LabelService, TorchLabelService, TrainingService, stride
...
def make_abspath(rel_path):
    if not os.path.isabs(rel_path):
        rel_path = str(Path(os.path.dirname(os.path.realpath(__file__)))/ rel_path)
    return rel_path

def load_service(config):
    model_path = str(Path(make_abspath(config["model_path"])))
...
def start(config):
    ...
    train_script = str(Path(make_abspath(config["train_script"])))
    speech_dataset_path = str(Path(make_abspath(config["speech_dataset_path"])))
