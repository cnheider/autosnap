...
from exclude.honk.model.manage_audio import AudioPreprocessor
...
class SpeechDataset(data.Dataset):
    ...
    def load_audio(self, example, silence=False):
        if silence:
            example = "__silence__"
        if random.random() < 0.7 or not self.set_type == DatasetSplit.TRAIN:
            try:
        ...
        if self.set_type == DatasetSplit.TRAIN:
    ...
    def splits(cls, config):
        ...
        for folder_name in os.listdir(folder):
            ...
            for filename in os.listdir(path_name):
                ...
                if bucket < dev_pct:
                    tag = DatasetSplit.DEV
                elif bucket < test_pct + dev_pct:
                    tag = DatasetSplit.TEST
                else:
                    tag = DatasetSplit.TRAIN
        ...
        datasets = (cls(sets[0], DatasetSplit.TRAIN, train_cfg), cls(sets[1], DatasetSplit.DEV, test_cfg),
                    cls(sets[2], DatasetSplit.TEST, test_cfg))
        return datasets
