...
from exclude.honk.model import model as mod
...
def print_eval(name, scores, labels, loss, end="\n"):
    ...
    loss = loss.item()
    print(f"{name} accuracy: {accuracy:>5}, loss: {loss:<25}", end=end)
    ...
    for epoch_idx in range(config["n_epochs"]):
        for batch_idx, (model_in, labels) in enumerate(train_loader):
            ...
            if step_no > schedule_steps[sched_idx]:
                sched_idx += 1
                print(f"changing learning rate to {config['lr'][sched_idx]}")
...
def main():
    output_file = str(Path(os.path.join(os.path.dirname(os.path.realpath(__file__)), "..", "model", "model.pt")))
    parser = argparse.ArgumentParser()
